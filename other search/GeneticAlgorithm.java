// copied from https://sites.google.com/site/indy256/algo/genetic_algorithm
// then edited by John MacCormick, 10/3/2014
...
public class GeneticAlgorithm extends JFrame implements Runnable {
	...
	public void geneticAlgorithm() {
		...
		for (int i = 0; i < populationLimit; i++)
			population.chromosomes.add(new Chromosome(
					optimize(getRandomPermutation(n))));
		...
		for (generation = 0; generation < generations; generation++) {
			...
			while (population.chromosomes.size() < population.populationLimit) {
				...
				int i2 = (i1 + 1 + rnd
						.nextInt(population.chromosomes.size() - 1))
						% population.chromosomes.size();
		...
		for (boolean improved = true; improved;) {
			...
			for (int i = 0; i < n; i++) {
				for (int j = 0; j < n; j++) {
					...
					int i1 = (i - 1 + n) % n;
					int j1 = (j + 1) % n;
					double delta = dist(x[res[i1]], y[res[i1]], x[res[j]],
							y[res[j]])
	...
	static class Population {
		...
		public void nextGeneration() {
			...
			chromosomes = new ArrayList<>(chromosomes.subList(0,
					(chromosomes.size() + 1) / 2));
		}
	}
		...
		setContentPane(new JPanel() {
			protected void paintComponent(Graphics g) {
				...
				g.drawString(String.format("length: %.3f", eval(bestState)), 5,
						h + 20);
				g.drawString(String.format("generation: %d", generation), 150,
						h + 20);
			}
		});
